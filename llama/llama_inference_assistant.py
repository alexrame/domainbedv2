...
import numpy as np
import llama_utils, llama_ppo_assistant, inference_utils, args_utils
...
class PredictorAssistant(inference_utils.Predictor):
    ...
    def get_rewards(self, texts):
        ...
        rewards = [
            [
                sentiment_pipe(
                    llama_ppo_assistant.transform_text_assistant(
...
class Samples:
    ...
    @staticmethod
    def get_fake_samples_assistant(bs):
    ...
    @staticmethod
    def get_samples_assistant(dataset_name, bs=16):
        ds = llama_ppo_assistant.build_dataset(
...
if __name__ == "__main__":
    if os.environ.get("MERGE", "0") == "0":
        default_args = args_utils.DefaultArgsInferenceAssistant()
    else:
        raise ValueError()
    ...
    if script_args.dataset_name == "samples":
        query_tensors = Samples.get_fake_samples_assistant(bs=script_args.num_samples)
    else:
        query_tensors = Samples.get_samples_assistant(
