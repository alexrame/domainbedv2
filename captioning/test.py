...
def get_wa_model(checkpoints_paths, map_location, coeffs):
    ...
    for i in range(num_models):
        ...
        if i == 0:
            wa_state_dict = {
                k: state_dict[k] * coeffs[i] for k in state_dict.keys()}
        else:
            keys = list(set(state_dict.keys()) & set(wa_state_dict.keys()))
            wa_state_dict = {
                k : state_dict[k] * coeffs[i] + wa_state_dict[k]
                for k in keys}
...
def test(
    ...
    if not ensemble_args.ensemble:
        ...
        # partially_load_state_dict(model=model, state_dict=checkpoint['model_state_dict'], verbose=True)
    else:
    ...
    if False:
        ...
        pred_dict, gts_dict = evaluate_model_on_set(
            ddp_model,
            coco_dataset.caption_idx2word_list,
            coco_dataset.get_sos_token_idx(),
            coco_dataset.get_eos_token_idx(),
...
def spawn_train_processes(
    ...
    show_predictions, num_gpus, ddp_sync_port, save_model_path
):
    ...
    mp.spawn(
        test,
        args=(
            num_gpus, is_end_to_end, model_args, ensemble_args, coco_dataset,
            eval_parallel_batch_size, eval_beam_sizes, show_predictions, array_of_init_seeds,
            max_sequence_length, save_model_path, ddp_sync_port
        ),
        nprocs=num_gpus,
        join=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_end_to_end', type=args_utils.str2bool, default=True)
    parser.add_argument('--ensemble', default="")
    parser.add_argument('--coeffs', type=args_utils.str2listf, default=[])

    parser.add_argument('--num_gpus', type=int, default=1)
    parser.add_argument('--ddp_sync_port', type=int, default=12354 + random.randint(0, 1000))
    parser.add_argument(
        '--save_model_path',
    ...
    ensemble_args = Namespace(
        ensemble=args.ensemble, coeffs=args.coeffs,
    )
