...
def train(
    ...
    if not train_args.reinforce:
        ...
        reinforce_reward = ReinforceCiderReward(
            ...
            rank,
            reinforce_metric=train_args.reinforce if not ONLY_TEST else 'cider'
        )
    ...
    for it in range(already_trained_steps, num_iter):
        ...
        if ((it + 1) % data_loader.get_num_batches()
            == 0) or ((it + 1) % train_args.eval_every_iter == 0) or ONLY_TEST:
            if not train_args.reinforce:
            ...
            time_to_save = True
        if ONLY_TEST:
            raise Exception("Only test")

        # saving
        elapsed_minutes = (time() - saving_timer_start) / 60
        if time_to_save and not ONLY_TEST:
...
def distributed_train(
    rank, world_size, model_args, optim_args, coco_dataset, array_of_init_seeds, model_max_len,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ddp_sync_port', type=int, default=12354 + random.randint(0, 100))
    parser.add_argument('--save_path', type=str, default=os.path.join(DATA_DIR, 'saves/'))
